...
tz = datetime(datestr(now)); 
tz.TimeZone = sprintf('%+03d:00', round(-157.858093 / 15))
tmzone = hours(tzoffset(tz)); tmzone = tmzone./24;
%% fpath

pth = pwd;
if ~contains(pth, 'gitwebsite');
    cd /Users/alliho/Documents/myrepos/gitwebsite/
end
...
disp('CDIP waves downloaded')
%%% tide

ids = {'alawa', 'maka', 'hono2', 'keehi', 'heeia', 'moku', 'halei', 'waian', 'brpt'};
for i=1:length(ids)
    disp([num2str(i) '/' num2str(length(ids)) ': ' ids{i}] )
    ioc = dload_ioc(ids{i}); % i think timezone is local, so imma convert to UTC
    % ioc.t = ioc.time - tmzone;
    iocs(i) = ioc;
end
disp('IOC tides downloaded')
...
t0 = now;
t0 = datenum(2026,8,16);

A = {cdip_south, cdip_windward, cdip_north, cdip_west};

for ai=1:length(A)
    cdip = A{ai};
    sfth = NaN(length(cdip.f), 72, length(cdip.time));
    for i=1:length(cdip.time)
        ...
        cdip.sfth(:,:,i) = mem_out.ds;
        cdip.th = mem_out.dir; 
    end
    % [~, i] = min(abs(cdip.time- (t0 + tmzone)));
    % t0 = cdip.time(i);
    % sp_data.freq = cdip.f'; 
    % sp_data.band_width = cdip.df'; 
    % sp_data.ener_dens = cdip.sf(:,i); 
    ...
    % sp_data.b2 = cdip.b2(:,i); 
    % [mem_out] = MEM_est(sp_data);
    % cdip.mem_out = mem_out;
    A{ai} = cdip;
end
[cdip_south, cdip_windward, cdip_north, cdip_west] = A{:};
% 
% 
% cdip = cdip_south;
% [~, i] = min(abs(cdip.time- (t0 + tmzone)));
...
[~, ti] = min(abs(cdip.time - t0)); 
th = cdip.th; fr = cdip.f; Efth = cdip.sfth(:,:,ti);
...
title({ ['CDIP' num2str(cdip.id)], [datestr(cdip.time(ti) + tmzone ,'yyyy/mm/dd HH:MM') ' PST'] })
...
%% [save] map, add direcitonal, tide indicators (clean)
tmax = max(cellfun(@max, {cdip_north.time, cdip_west.time, cdip_south.time, cdip_windward.time}));  % this is in UTC


% for tmax = ([datenum(2026,8,15):3/24:datenum(2026,8,18)]-tmzone)
% for tmax = ([datenum(2026,8,18):3/24:datenum(2026,8,19)]-tmzone)
% for tmax = ([datenum(2026,8,14):3/24:datenum(2026,8,15)]-tmzone)

% A_cdip = {cdip_south, cdip_north, cdip_windward, cdip_west};
A_cdip = {cdip_west cdip_windward cdip_south cdip_north};
    ...
    %%% TIDE LOCATIONS AND INDICATORS
    for io = 1:length(iocs); 
        ioc = iocs(io); 
        minN = [(1/24)./mode(diff(ioc.time))];
        tt = find(isin(ioc.time, [tmax - 3/24 tmax])); 
    ...
    %%% CDIP LOCATIONS
    for ai=1:length(A_cdip);
        cdip = A_cdip{ai};
        plot(cdip.lon0, cdip.lat0, 'ro', 'LineWidth',2, 'Color', 'r', 'MarkerFaceColor', 'w', 'MarkerSize',8, 'DisplayName', ['CDIP' num2str(cdip.id)])
% , ...
%             'MarkerFaceColor',buoycol.*0.6 + [buoycol - nanmean(buoycol)].*1.4)
        txtoffst = 0.015 * sign(cdip.lat0-21.4); 
                text(cdip.lon0+txtoffst*2, cdip.lat0+txtoffst, ['CDIP' num2str(cdip.id)], 'FontSize',10, 'FontName','avenir', 'color', 'r', ...
            'HorizontalAlignment', 'center', 'VerticalAlignment','middle')
    end
    % -------------------------------------------------------------------------
    % SET UP DIRECTIONAL AXES LOCATIONS
    % -------------------------------------------------------------------------
    A_cdip_ha = cell(size(A_cdip));
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    for ai=1:length(A_cdip)
        cdip = A_cdip{ai};
        % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        thisha = A_cdip_ha{ai}; axes(thisha); hold on;
        colormap(gca, jet); caxis([-6.5 -1.5])
        [~, ti] = min(abs(cdip.time - tmax));
        th = cdip.th; fr = cdip.f; Efth = cdip.sfth(:,:,ti);
    ...
    textbypos(ha(1).Position(1) + ha(1).Position(3)*0.007, ha(1).Position(2) + ha(1).Position(4)*0.02, ['Updated ' datestr(tmax+tmzone, 'yyyy/mm/dd HH:MM') 'HST'] , 'fontname', 'avenir', 'Color', [1 1 1].*0.7)
    


    savejpg(gcf, 'howsthewaves_map_oahu', [upath wpath], 'on')
    savejpg(gcf, ['howsthewaves_map_oahu_' datestr(tmax+tmzone, 'yyyymmddTHHMMSS') 'HST'], [upath wpath], 'on')
...
%% finish
disp('-----------------------------------------')
disp('Successfully executed howsthewater_oahu.m')
disp('-----------------------------------------')
